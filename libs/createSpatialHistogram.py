...
def compSpatialHist(x,exist,kSize=64,sSize=4,isNormMode='sum',thre=0.05):
    # binarize images
    x_bin = x*exist
...
if __name__ == "__main__":
    ...
    for i in range(plotNum):
        ...
        ax.imshow(x[i,:,:,0],vmin=0,vmax=1,cmap="jet")
        ax.set_title('dense')

        ax=fig.add_subplot(2,2,2)
        im = ax.imshow(x_mask[i,:,:,0],vmin=0,vmax=1,cmap="jet")
